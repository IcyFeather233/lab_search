...
def handle_one_file(filename):
    ...
    while line:
        ...
        # 如果是点赞行及以下的行，就跳过
        if line.replace('/r', '').replace('/n', '').strip().isdigit() or line == '':
            print("是点赞行及一下的行" + line)
            line = f.readline()
            continue

        # 如果是点赞行以上的行，就找到下面的点赞行
        zan_num = zan.zan_num(line, f).strip()
        print("点赞数为" + zan_num)
        ...
        for e in data:
            if e != '':
                ...
                if linkdata[0]:
                    link = linkdata[1]
                    print(link)
                    mypoem.crawl_the_poetry(link, e, filename, zan_num)
                print("-------------")
